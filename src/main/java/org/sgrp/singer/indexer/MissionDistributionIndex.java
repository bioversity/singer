...
public class MissionDistributionIndex extends BaseIndexer implements IndexerInterface {
	protected static MissionDistributionIndex	mgr	= null;
	
	private static Logger LOG = Logger.getLogger(Main.class);

	public static MissionDistributionIndex getInstance() {
		if (mgr == null) {
			mgr = new MissionDistributionIndex();
		}
		return mgr;
	}
	...
	public static void indexFromMain(Map<String, String> options)throws Exception
	{
		...
		MissionDistributionIndex mdIndex = new MissionDistributionIndex();
	...
	public static void main(String args[]) throws Exception {
		...
		MissionDistributionIndex dIndex = new MissionDistributionIndex();
		String sql = "select * from cmtransdata";
	...
	public String						INDEX_DIR		= "missdistindex";
	...
	public MissionDistributionIndex() {
		super();
	...
	public int getRowCount(Connection con, String sql) {
		//String countSql = "select count(*) from cmtransdata";
		sql=sql.toLowerCase();
	...
	public String getShortName() {
		return "Collecting Mission Distribution(s)";
	}
	...
	public synchronized void indexData(String sql, boolean fullrefresh) throws Exception {
		...
		try {
			//String normSql = "select * from cmtransdata";
			...
			try {
				...
				for (int i = 0; i < pages; i++) {
					...
					while (rs.next()) {
						...
						String tinstcode = rs.getString("tinstcode");
						String id = code + sp + tinstcode;
						...
						map.put("sp", sp);
						map.put("tinstcode", tinstcode);
						map.put("samp", rs.getString("samp"));
						map.put("faocode", rs.getString("faocode"));
						map.put("acronym", rs.getString("acronym"));
						map.put("name", rs.getString("name"));
						map.put("street", rs.getString("street"));
						map.put("city", rs.getString("city"));
						map.put("zip", rs.getString("zip"));
						map.put("tlf", rs.getString("tlf"));
						map.put("tlx", rs.getString("tlx"));
						map.put("fax", rs.getString("fax"));
						map.put("ema", rs.getString("ema"));
	...
	public synchronized void indexCMDistribution(IndexWriter writer, Map<String, String> map, boolean fullrefresh) throws Exception {
		try {
			...
			LOG.info("Indexing Mission Distribution :" + map.get(ID));
			...
			while (itr.hasNext()) {
				...
				if (key.equals(ID) || key.equals(NAME) || key.equals("tinstcode")) {
