...
public class DistributionIndex extends BaseIndexer implements IndexerInterface {
	protected static DistributionIndex	mgr	= null;
	
	private static Logger LOG = Logger.getLogger(Main.class);

	public static DistributionIndex getInstance() {
		if (mgr == null) {
			mgr = new DistributionIndex();
		}
		return mgr;
	}
	...
	public static void indexFromMain(Map<String, String> options)throws Exception
	{
		...
		DistributionIndex dIndex = new DistributionIndex();
		LOG.info("Sql here is :" + sql);
		dIndex.indexData(sql, !update);
	...
	public static void main(String args[]) throws Exception {
		...
		DistributionIndex dIndex = new DistributionIndex();
		String sql = "select * from transdata";
	...
	public String						INDEX_DIR		= "distindex";
	...
	public synchronized void indexData(String sql, boolean fullrefresh) throws Exception {
		...
		try {
			//String normSql = "select * from transdata";
			...
			try {
				...
				for (int i = 0; i < pages; i++) {
					...
					while (rs.next()) {
						...
						String recptcode = rs.getString("recptcode_");
						Timestamp transdate = null;
						try {
							transdate = rs.getTimestamp("datetrans");
						} catch (Exception e) {

						}
						String id = accenumb + recptcode;
						if (transdate != null) {
							String dateStr = GenericForm.getlongAsDate(transdate.getTime(), "ddMMyyyy");
							id = id + dateStr;
							map.put("year", GenericForm.getlongAsDate(transdate.getTime(), "yyyy"));
						} else {
							map.put("year", "0000");
						}
						...
						map.put("recptcode", mangleKeywordValue(recptcode.toLowerCase()));
						map.put("ctycode", mangleKeywordValue(rs.getString("ctycode_")));
						map.put("usercode", mangleKeywordValue(rs.getString("usercode_")));
						map.put("devstat", mangleKeywordValue(rs.getString("devstat")));
						...
						map.put("region", mangleKeywordValue(rs.getString("region")));
						if (transdate != null) {
							map.put("datetrans", rs.getDate("datetrans") + "");
						} else {
							map.put("datetrans", "");
						}
						map.put("repdate", rs.getDate("repdate") + "");
						// map.put(AccessionConstants.COLLECTIONCODE,AccessionConstants.COLLECTION+rs.getString("collcode_"));
	...
	public synchronized void indexDistribution(IndexWriter writer, Map<String, String> map, boolean fullrefresh) throws Exception {
		try {
			...
			LOG.info("Indexing Distribution :" + map.get(ID));
			...
			while (itr.hasNext()) {
				...
				if (!(key.equals("year") || key.equals("ctycode") || key.equals("region") || key.equals("devstat") || key.equals("usercode") || key.equals(AccessionConstants.TAXONCODE) || key.equals(AccessionConstants.GENUSCODE) || key
					.equals(AccessionConstants.SPECIESCODE))) {
					sb.append(AccessionConstants.makeFormattedString(key, value));
				}
				// sb.append("["+key+":"+value+"]");
				if (key.equals(ID) || key.equals(NAME) || key.equals("year") || key.equals("ctycode") || key.equals("region") || key.equals("devstat") || key.equals("usercode") || key.equals(AccessionConstants.TAXONCODE)
					|| key.equals(AccessionConstants.GENUSCODE) || key.equals(AccessionConstants.SPECIESCODE)) {
